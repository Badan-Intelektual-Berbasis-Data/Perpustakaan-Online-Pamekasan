...
useEffect(() => {
  setBooksData(booksData.filter((book) => book.title.includes(searchValue) || book.author_name.includes(searchValue)))
    ...
    >
      ...
      {!authenticated && (
            <div className="flex gap-x-2">
              <Link className="text-white bg-black border-white border-2 text-sm px-6 py-2" to="/login">Masuk</Link>
              <Link className="text-black bg-white border-black border-2 text-sm px-6 py-2" to="/register">Daftar</Link>
            </div>
          )}
          {authenticated && 
          <Link to="/profile">
